...
const emptyEvaluationScope = {
  id: null, row_version: null, source_classification: null, rendered_prose: null, limitation_text: null,
  editable: false, read_only_reason: "Chưa có phạm vi đánh giá canonical cho hồ sơ này.",
  taxonomy_version_id: null, gxp_type: "", blocks: [], taxonomy_nodes: [],
};
...
function initialExpandedNodeIds(caseWorkspace: CaseWorkspace): Set<string> {
  const scope = caseWorkspace.evaluation_scope;
...
export function EvaluationScopeWorkspace({ caseWorkspace, onSave }: {
  ...
  const [expanded, setExpanded] = useState<Set<string>>(() => initialExpandedNodeIds(caseWorkspace));
  ...
  useEffect(() => {
    if (!editing) {
      ...
      setActiveBlock(0);
      setExpanded(initialExpandedNodeIds(caseWorkspace));
      setError(null);
    }
  }, [caseWorkspace, editing, scope.limitation_text]);

  if (!scope.id) return <p className="workspace-note">{scope.read_only_reason}</p>;
  if (scope.source_classification === "PROSE_ONLY") return <section className="workspace-section"><h4>Phạm vi đánh giá</h4><pre className="evaluation-scope-prose">{scope.rendered_prose}</pre></section>;
  ...
  const removeActiveBlock = () => { if (draft.length > 1) { setDraft((current) => current.filter((_, index) => index !== activeBlock)); setActiveBlock((current) => Math.max(0, current - 1)); } };
  async function save() {
  ...
  return <section className="workspace-section evaluation-scope-workspace">
    ...
    {block ? <div className="evaluation-scope-block" onKeyDown={(event) => {
      if (!editing && scope.editable && (event.key === "Enter" || event.key === "F2")) { event.preventDefault(); setEditing(true); }
      if (editing && event.key === "Escape" && !pending) { event.preventDefault(); cancel(); }
    }} tabIndex={scope.editable ? 0 : undefined}>
      <div className="evaluation-scope-block-header">
        <label>Tên phạm vi<input disabled={!editing} value={block.name} onChange={(event) => updateBlock(activeBlock, (current) => ({ ...current, name: event.target.value }))} /></label>
        <label>Ghi chú<textarea disabled={!editing} value={block.note} onChange={(event) => updateBlock(activeBlock, (current) => ({ ...current, note: event.target.value }))} /></label>
      </div>
      <div aria-label="Cây phạm vi đánh giá" className="evaluation-scope-tree" role="tree">
        {scope.taxonomy_nodes.map((node) => {
          ...
          return <div aria-expanded={hasChildren ? expanded.has(node.id) : undefined} className={`evaluation-scope-node depth-${Math.min(depth, 2)} ${isSelected ? "is-selected" : ""}`} key={node.id} onDoubleClick={() => scope.editable && setEditing(true)} role="treeitem" style={{ paddingInlineStart: `${depth * 1.1}rem` }} title={node.hint ?? undefined}>
            ...
            <button aria-checked={state === "mixed" ? "mixed" : state === "checked"} aria-label={`Chọn ${node.key}`} className={`tree-check ${state}`} disabled={!editing} onClick={() => toggle(node.id)} onKeyDown={(event) => { if (editing && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); toggle(node.id); } }} role="checkbox" type="button"><span /></button>
            <span className="tree-key">{node.key}</span>
            <div className="tree-copy">
              <span className="tree-taxonomy-description">{node.description}</span>
              {isSelected && customDescription ? <p className="tree-custom-description">{customDescription}</p> : null}
              {isSelected && editing ? <textarea aria-label={`Mô tả tùy chỉnh ${node.key}`} className="tree-custom-input" onChange={(event) => updateBlock(activeBlock, (current) => ({ ...current, selected: { ...current.selected, [node.id]: event.target.value } }))} placeholder="Mô tả tùy chỉnh (để trống nếu không có)" value={customDescription} /> : null}
            </div>
          </div>;
        })}
      </div>
      {scope.blocks[activeBlock]?.unkeyed_entries.map((item) => <p className="workspace-note" key={item.source_order}>{item.text}</p>)}
      <label className="evaluation-scope-limitation">Giới hạn<textarea disabled={!editing} value={limitation} onChange={(event) => setLimitation(event.target.value)} /></label>
    </div> : null}
    {!editing && scope.editable ? <p className="workspace-note">Nhấp đúp vào node hoặc nhấn Enter/F2 để chỉnh sửa. Escape hủy thay đổi chưa lưu.</p> : null}
