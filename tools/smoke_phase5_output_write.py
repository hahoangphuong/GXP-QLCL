...
from backend.app.db.enums import CaseState, DocumentGenerationStatus
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with session_factory() as session:
            ...
            allocated = prepare_and_allocate_document_generation_job(
                ...
                DocumentPreparationInput(
                    request=DocumentGenerationRequest(
                        ...
                        idempotency_key="smoke-phase5-output-write",
                    ),
                    payload_values={
                        "QDKT": "12/QD",
                        "NgayQDKT": "2024-01-15",
            ...
            checksum = finalize_output_document_version_write(
                ...
                binary_payload=b"rendered-qdkt-docx",
            )
            document_version = session.execute(
                select(DocumentVersion).where(DocumentVersion.id == allocated.output_allocation.document_version_id)
            ).scalar_one()
            generation_run = session.execute(
                select(DocumentGenerationRun).where(DocumentGenerationRun.id == allocated.output_allocation.generation_run_id)
            ).scalar_one()
            session.commit()

        written_path = inspection_root.joinpath(*allocated.output_allocation.storage_relative_path.split("/"))
        written_text = written_path.read_bytes().decode("utf-8")
        ...
        json.dumps(
            {
                ...
                "status_matches_expected": generation_run.status == DocumentGenerationStatus.SUCCEEDED,
            },
            ensure_ascii=False,
            indent=2,
        )
