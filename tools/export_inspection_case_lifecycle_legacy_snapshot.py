...
def _select_rows(
    ...
    *,
    ...
    for raw in source_rows:
        ...
        if require_case_link and not str(normalized.get("inspection_case_legacy_id_ref", "")).strip():
            continue
        _required_integer(normalized.get("ID", ""), source_sheet=source_sheet, source_row=source_row, field="ID")
        if require_case_link:
            _required_integer(
                normalized.get("inspection_case_legacy_id_ref", ""),
...
def build_snapshot_payload(workbook: Path, source_rows: dict[str, list[dict[str, str]]]) -> dict[str, Any]:
    ...
    ktra_rows = _select_rows(
        source_rows["db.ktra"], SNAPSHOT_KTRA_FIELDS, source_sheet="db.ktra", require_case_link=False
    )
    cc_rows = _select_rows(
        source_rows["db.cc"], SNAPSHOT_CC_FIELDS, source_sheet="db.cc", require_case_link=True
    )
    sections = {
