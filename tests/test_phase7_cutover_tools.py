...
def _write_checklist(tmp_path: Path, *, status: str = "pending") -> None:
    ...
    _write_json(tmp_path / "evidence" / "cutover_execution_checklist.json", {"items": items})


def _build_readiness(tmp_path: Path) -> dict:
    return readiness.build_readiness(evidence_dir=tmp_path / "evidence")
...
def test_rollback_pending_and_malformed_checklist_block(tmp_path: Path, monkeypatch) -> None:
    ...
    (tmp_path / "evidence" / "cutover_execution_checklist.json").write_text("{invalid", encoding="utf-8")
    report = _build_readiness(tmp_path)
