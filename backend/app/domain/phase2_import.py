...
FIELD_ALIASES = {
    ...
    "facility_leader_name": {"NGƯỜI ĐỨNG ĐẦU CƠ SỞ"},
    ...
    "inactive_status_text": {"NGỪNG HOẠT ĐỘNG"},
    "linked_certificate_ids": {"ID CC"},
    "business_eligibility_decision_reference": {"Số QĐ cấp"},
    "business_eligibility_issuance_sequence_text": {"LẦN CẤP"},
    "business_eligibility_issuance_history_text": {"LỊCH SỬ CẤP"},
    "business_eligibility_professional_license_number": {"CHỨNG CHỈ HÀNH NGHỀ"},
    "business_eligibility_professional_license_issued_on": {"NGÀY CẤP CCHN"},
    "business_eligibility_professional_license_issuer": {"NƠI CẤP CCHN"},
    "business_eligibility_responsible_license_issued_on": {"NGÀY CẤP CCHN - PTCM"},
    "business_eligibility_responsible_license_issuer": {"NƠI CẤP CCHN - PTCM"},
    "business_eligibility_business_activity_text": {"HOẠT ĐỘNG KINH DOANH"},
    "business_eligibility_professional_qualification_text": {"TRÌNH ĐỘ CHUYÊN MÔN"},
    "business_eligibility_handled_by_name": {"NGƯỜI XỬ LÝ"},
    "business_eligibility_application_dossier_reference": {"HỒ SƠ ĐỀ NGHỊ CẤP/ĐIỀU CHỈNH"},
...
LENGTH_AUDIT_RULES: tuple[LengthAuditRule, ...] = (
    ...
    LengthAuditRule("db.dkkd", "Mã số CC", BusinessEligibilityVersion, "certificate_number", "identifier", lambda row: row.get("certificate_number")),
    LengthAuditRule(
        "db.dkkd",
    ...
    *,
    ...
    for raw_row in snapshot["db.cso"]:
        ...
        expected_fields = {
            ...
            "facility_leader_name": row.get("facility_leader_name") or None,
            ...
            "current_status_text": row.get("inactive_status_text") or None,
        }
        entity = _ensure_entity(
            session,
            stats,
    ...
    for raw_row in snapshot["db.ktra"]:
        ...
        outcome = _ensure_single_child(
            ...
            expected_fields={
                ...
                "inspection_period_state": inspection_period_result.state.value,
    ...
    for raw_row in snapshot["db.dkkd"]:
        ...
        expected_fields = {
            ...
            "replaces_legacy_dkkd_id": parse_int(row.get("THAY THẾ GIẤY ID", "")),
            "replaced_by_legacy_dkkd_id": parse_int(row.get("BỊ THAY THẾ BỞI GIẤY ID", "")),
        }
        entity = _ensure_entity(
            session,
            stats,
        ...
        version = _ensure_single_child(
            ...
            expected_fields={
                ...
                "version_no": 1,
                "certificate_number": row.get("certificate_number") or None,
                "issued_on": parse_date(row.get("certificate_issue_date", "")),
                "expires_on": None,
                "professional_responsible_person_name": row.get("professional_responsible_person_name") or None,
                "quality_assurance_person_name": row.get("quality_assurance_person_name") or None,
                "professional_qualification_text": row.get("business_eligibility_professional_qualification_text") or None,
                "professional_license_number": row.get("business_eligibility_professional_license_number") or None,
                "professional_license_issued_on": parse_date(row.get("business_eligibility_professional_license_issued_on", "")),
                "professional_license_issuer": row.get("business_eligibility_professional_license_issuer") or None,
                "responsible_license_issued_on": parse_date(row.get("business_eligibility_responsible_license_issued_on", "")),
                "responsible_license_issuer": row.get("business_eligibility_responsible_license_issuer") or None,
                "decision_reference": row.get("business_eligibility_decision_reference") or None,
                "issuance_sequence_text": row.get("business_eligibility_issuance_sequence_text") or None,
                "issuance_history_text": row.get("business_eligibility_issuance_history_text") or None,
                "business_activity_text": row.get("business_eligibility_business_activity_text") or None,
                "current_status_text": None,
                "handled_by_name": row.get("business_eligibility_handled_by_name") or None,
                "application_dossier_reference": row.get("business_eligibility_application_dossier_reference") or None,
